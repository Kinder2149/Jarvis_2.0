# TEST-02 — Écrire les tests E2E pour les missions FRONT-01 à 06

> Lire PROJET_CONTEXTE.md en entier avant toute action.
> Prérequis : TEST-01 terminé avec VERDICT ✅

---

## Objectif

Créer le fichier `tests/test_front_ux.py` qui valide les 6 missions de UX Refactoring.
Les fixtures Playwright (`page`, `browser`) sont déjà dans `tests/conftest.py` — ne pas les redéfinir.

---

## AVANT D'AGIR

1. Confirmer que `tests/conftest.py` contient bien la fixture `page`
2. Confirmer que `playwright` est installé : `python -c "from playwright.sync_api import sync_playwright; print('OK')"`
3. Lister les sessions disponibles (résultat de TEST-01 ÉTAPE 4)
4. Attendre confirmation

---

## FICHIER À CRÉER : `tests/test_front_ux.py`

Créer exactement ce fichier :
...
class TestFront01CTAModuleCode:
    ...
    def test_cta_bouton_retour_projet_present(self, page: Page):
        ...
        # Chercher un lien vers project.html dans la zone CTA
        link = action_zone.query_selector("a[href*='project.html']")
    ...
    def test_regression_waiting_validation_toujours_fonctionnel(self, page: Page):
        ...
        session_id, project_id = get_session_by_status("WAITING_VALIDATION")
        if not session_id:
            pytest.skip("Aucune session WAITING_VALIDATION disponible")
...
class TestFront02CTAAtelier:
    ...
    def test_atelier_charge_sans_erreur_js(self, page: Page):
        ...
        assert page.query_selector("#page-atelier") is not None, "#page-atelier absent"
...
class TestFront04KanbanBadges:
    ...
    def test_icones_statut_presentes_dans_kanban(self, page: Page):
        ...
        # Au moins une icône de statut doit être présente
        status_icons = ["⏸️", "✅", "❌", "⚙️", "⛔"]
        has_any = any(icon in kanban_html for icon in status_icons)
        assert has_any, \
            f"Aucune icône de statut session dans le kanban. " \
            f"Icônes attendues : {status_icons}. HTML (200 chars) : {kanban_html[:200]}"
...
class TestFront06ProjectInline:
    ...
    def test_nouveau_chat_sans_modal(self, page: Page):
        ...
        assert btn is not None, "#btn-new-chat absent sur project.html"
        ...
        # Vérifier qu'aucune modal ne s'est ouverte
        modal = page.query_selector(".modal-overlay")
        assert modal is None, \
            "Modal ouvert après clic 'Nouveau Chat' depuis project.html — " \
    ...
    def test_nouveau_module_modal_projet_prerempli(self, page: Page):
        ...
        btn = page.query_selector("#btn-new-module")
        assert btn is not None, "#btn-new-module absent sur project.html"
        ...
        if project_select:
            is_disabled = project_select.is_disabled()
            assert is_disabled, \
                "Select projet non désactivé — le projet courant doit être pré-rempli et grisé (FRONT-06)"
...
Après avoir créé le fichier, lancer :

```bash
pytest tests/test_front_ux.py -v --no-header 2>&1
```

Résultats attendus :
- **passed** → test validé ✅
- **SKIPPED** → données absentes, normal
- **FAILED** → bug à corriger avant de passer à TEST-03

**Règle : tout FAILED doit être corrigé immédiatement dans le fichier JS concerné avant de continuer.**

Si une correction nécessite une décision architecturale (plus de 2 fichiers impactés ou logique ambiguë) : **STOP et revenir vers Claude avec le détail exact du FAILED**.

---

## FIN DE MISSION

- [ ] `tests/test_front_ux.py` créé
- [ ] `pytest tests/test_front_ux.py -v` → 0 FAILED (des SKIP sont OK)
- [ ] PROJET_CONTEXTE.md section 8 mis à jour
- [ ] CHANGELOG.md : ligne "Tests E2E UX Refactoring — 6 missions FRONT couvertes"
