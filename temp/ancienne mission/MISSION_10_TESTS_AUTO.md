# MISSION 10 — Tests Automatiques Frontend E2E
...
MISSION : Tests automatiques E2E du Frontend V2

OBJECTIF :
Écrire et exécuter une suite de tests automatiques End-to-End (E2E) qui valide
l'ensemble du frontend V2 JARVIS : chaque page, chaque bouton clé, chaque flux utilisateur.
Les tests tournent avec Playwright (Python) contre le serveur déjà actif sur localhost:8000.
Aucun test manuel ne sera fait — ces tests automatiques sont la validation finale.

Le serveur JARVIS est déjà démarré sur http://localhost:8000.
Ne pas redémarrer le serveur, ne pas modifier le backend.

PÉRIMÈTRE :
Fichiers à créer :
- tests/test_frontend_e2e.py   ← suite de tests Playwright complète
- tests/conftest.py             ← fixtures Playwright partagées

Fichiers à modifier :
- AUCUN fichier frontend ou backend ne doit être modifié dans cette mission.
  Si un test échoue à cause d'un bug, noter le bug dans le rapport final.
  Les corrections seront dans une mission séparée.

---

## ÉTAPE 1 — INSTALLATION PLAYWRIGHT
...
Créer `tests/test_frontend_e2e.py` avec les tests suivants.
...
class TestServeur:
    ...
    def test_static_files_servis(self):
        ...
        for path in ["index.html", "chat.html", "project.html", "module-code.html", "settings.html"]:
...
class TestModuleCode:
    ...
    def test_module_code_lien_projet(self, page: Page):
        ...
        assert f"project.html?id={project_id}" in href
...
class TestExplorateur:
    ...
    def test_explorateur_visible_si_dossier_lie(self, page: Page):
        ...
        page.goto(f"{BASE_URL}/project.html?id={project_with_path['id']}")
        page.wait_for_timeout(3000)
...
class TestNavigation:
    ...
    def test_navigation_breadcrumb_projet(self, page: Page):
        """Le badge projet dans le chat navigue vers project.html."""
        project_id = get_first_project_id()
        if not project_id:
            pytest.skip("Aucun projet disponible")
        ...
        if badge:
            badge.click()
            page.wait_for_timeout(1500)
            assert "project.html" in page.url
    ...
    def test_toutes_pages_sans_erreur_js(self, page: Page):
        ...
        if project_id:
            pages_to_test.append(f"{BASE_URL}/project.html?id={project_id}")
        if conv_id:
...
Après exécution, produire un rapport structuré :
...
RÉSULTAT GLOBAL : X tests passés / Y tests total
...
TESTS ÉCHOUÉS (si applicable) :
  - [nom_test] : [raison de l'échec]
  - ...

BUGS CONFIRMÉS À CORRIGER :
  - [description bug + fichier + ligne]

TESTS SKIPPÉS (données manquantes) :
  - [nom_test] : [raison skip]
═══════════════════════════════════════════
```

**IMPORTANT** : NE PAS corriger les bugs trouvés pendant cette mission.
Les noter dans le rapport. Les corrections seront faites en Mission 11.
