...
class ProviderFactory:
    ...
    def create(agent_name: str, api_key: str = None) -> BaseProvider:
        ...
        Args:
            ...
            api_key: Clé API optionnelle (si None, utilise .env)
        ...
        # Vérifier si provider en cache (uniquement si pas de clé API custom)
        ...
        # Créer provider Gemini avec clé API spécifique
        provider = ProviderFactory._create_gemini(agent_name, api_key=api_key)

        # Mettre en cache (uniquement si pas de clé API custom)
        if not api_key:
